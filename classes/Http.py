    ...
    def getxml(self, url):
        try:
            ...
            with open(file, "w") as fh:  # open the file handler for logging
                ...
                print(f'{self.style.FILE}Log file written as {self.style.RED}"{file}"{self.style.LMGE} ({self.style.RED}{len(tags)}{self.style.RST} URLs discovered{self.style.LMGE}){self.style.RST}')
